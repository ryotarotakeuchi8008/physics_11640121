...
def load_dataset2(filename, batch_size):
    ...
    x_list_train = x_numpy_array_train.tolist()  # numpy型からlist型
    count = len(x_list_train) - batch_size
    # 入力データの作成
    data_train = [x_list_train[idx:idx + batch_size] for idx in range(count)]
    ...
    # 正解ラベルの作成
    labels_train = [x_list_train[idx + batch_size] for idx in range(count)]
    labels_train = torch.tensor(labels_train, dtype=torch.float)
    labels_train = labels_train.reshape(num_batch, -1)

    # validate用データの作成
    x_numpy_array_validate = myarray[0:100, 1]  # numpyの多次元配列のスライシング機能(列でスライス)
    x_list_validate = x_numpy_array_validate.tolist()  # numpy型からlist型
    count = len(x_list_validate) - batch_size
    # 入力データの作成
    data_validate = [x_list_validate[idx:idx + batch_size] for idx in range(count)]
    ...
    # 正解ラベルの作成
    labels_validate = [x_list_validate[idx + batch_size] for idx in range(count)]
